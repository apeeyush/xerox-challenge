...
def gen_val_data():
    df_id_age = pd.read_csv(sys.argv[1])
    df_id_time_labs = pd.read_csv(sys.argv[2])
    df_id_time_vitals = pd.read_csv(sys.argv[3])
    df_id_labs_vitals_merge = pd.merge(df_id_time_labs, df_id_time_vitals, how='inner', on=['ID','TIME'])
...
def predict(X_train_data,y_train_data,test_data,ids,times,i,j):
    ...
    # Prepare submission
    predictions_file = open("ICU_out/output_final_test_"+str(i)+"_"+str(j)+".csv", "wb")
